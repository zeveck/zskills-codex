...
if [ "${ZSKILLS_ALLOW_CLAUDE_PRESET:-}" != "1" ]; then
  echo "apply-preset: this bundled helper edits .claude runtime files and is disabled in the Codex port." >&2
  echo "Set ZSKILLS_ALLOW_CLAUDE_PRESET=1 only for explicit legacy Claude compatibility work." >&2
  exit 3
fi
