#!/bin/bash
# scripts/worktree-add-safe.sh BRANCH_NAME WORKTREE_PATH [BASE_BRANCH=main] [REMOTE=origin]
...
if ! git rev-parse --verify --quiet "$BRANCH_NAME" >/dev/null; then
  if ! git rev-parse --verify --quiet "$REMOTE/$BRANCH_NAME" >/dev/null; then
...
if [ "${ZSKILLS_ALLOW_BRANCH_RESUME:-}" != "1" ]; then
  ...
  echo "       To abort and delete the stale refs after explicit approval: git branch -D $BRANCH_NAME && git push $REMOTE --delete $BRANCH_NAME" >&2
  exit 4
fi
