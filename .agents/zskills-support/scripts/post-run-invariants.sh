...
#   7. Local base reconcilable with configured remote/base (warn, not fail — user may
...
if [ -n "$BRANCH_NAME" ] && [ "$BRANCH_NAME" != "$BASE_BRANCH" ] && [ "$BRANCH_NAME" != "main" ] && [ "$BRANCH_NAME" != "master" ]; then
...
if [ -n "$BRANCH_NAME" ] && [ "$LANDED_STATUS" = "landed" ]; then
  if git -C "$MAIN_ROOT" ls-remote --exit-code --heads "$REMOTE" "$BRANCH_NAME" >/dev/null 2>&1; then
...
# 7. Local base reconcilable with configured remote/base — WARN not FAIL
...
# invisibly.
if ! git -C "$MAIN_ROOT" fetch "$REMOTE" "$BASE_BRANCH" 2>/dev/null; then
  echo "INVARIANT-WARN (#7): 'git fetch $REMOTE $BASE_BRANCH' failed — cannot verify local base alignment" >&2
  ...
  if [ "$TREE_DIFF" -eq 0 ]; then
    echo "INVARIANT-WARN (#7): local $BASE_BRANCH has commits absent from $REMOTE/$BASE_BRANCH but tree is identical (squash-merge divergence). Manual reconciliation is required; reset only after explicit user approval." >&2
  else
