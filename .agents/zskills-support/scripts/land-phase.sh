...
WORKTREE_PATH="$1"
BASE_BRANCH="${ZSKILLS_BASE_BRANCH:-main}"
REMOTE="${ZSKILLS_REMOTE:-origin}"
...
if [ -d "$WORKTREE_PATH/.zskills/logs" ]; then
  if ! mkdir -p "$MAIN_ROOT/.zskills/logs"; then
    echo "ERROR: Could not create $MAIN_ROOT/.zskills/logs — aborting cleanup to preserve logs"
    exit 1
  fi
  for log in "$WORKTREE_PATH/.zskills/logs/"*.md; do
    [ -f "$log" ] || continue
    if [ ! -f "$MAIN_ROOT/.zskills/logs/$(basename "$log")" ]; then
      if ! cp "$log" "$MAIN_ROOT/.zskills/logs/"; then
...
# reshaped into a squash commit on the configured remote/base; the original commits are
# not reachable from the local base as individual objects, so `git branch -d`
# refuses. We use -D because the content is safe on the configured remote/base.
# For other statuses (pr-ready, pr-ci-failing, conflict), we keep the
# branch — work isn't fully landed yet.
if [ -n "$BRANCH" ] && [ "$BRANCH" != "$BASE_BRANCH" ] && [ "$BRANCH" != "main" ] && [ "$BRANCH" != "master" ] && [ "$BRANCH" != "HEAD" ]; then
...
if [ -n "$BRANCH" ] && [ "$BRANCH" != "$BASE_BRANCH" ] && [ "$BRANCH" != "main" ] && [ "$BRANCH" != "master" ] && [ "$BRANCH" != "HEAD" ]; then
  if echo "$LANDED_CONTENT" | grep -q "^status: landed"; then
    git -C "$MAIN_ROOT" ls-remote --exit-code --heads "$REMOTE" "$BRANCH" >/dev/null 2>&1
    LS_RC=$?
    case "$LS_RC" in
      0)
        # Branch exists on origin — delete it
        if ! git -C "$MAIN_ROOT" push "$REMOTE" --delete "$BRANCH"; then
          echo "ERROR: git push $REMOTE --delete $BRANCH failed"
        ...
        git -C "$MAIN_ROOT" ls-remote --exit-code --heads "$REMOTE" "$BRANCH" >/dev/null 2>&1
        if [ $? -eq 0 ]; then
          echo "ERROR: remote branch $BRANCH still on $REMOTE after --delete push"
          exit 1
        fi
        ;;
      2)
        # --exit-code: branch genuinely absent on origin — safe to skip
        echo "Remote branch $BRANCH already absent on $REMOTE — skipping delete."
        ;;
      *)
        ...
        echo "ERROR: git ls-remote for $BRANCH failed with exit $LS_RC — $REMOTE unreachable, misconfigured, or auth failure" >&2
