...
`update-zskills` was originally an installer for Claude Code. In this Codex conversion, do not execute its Claude-specific installation steps literally. Use it as a maintenance/reference workflow: inspect upstream assets, update `~/.codex/skills/<skill-name>/SKILL.md`, update support files under `/home/vscode/.codex/zskills-support`, and preserve Codex frontmatter plus these adaptation notes. Do not create or rely on `.claude/settings.json` as a Codex runtime configuration file.
