...
4. Run adversarial planning rounds. When Codex sub-agents are available, use separate agents for research, plan review, and devil's-advocate critique. If sub-agents are unavailable, run the same roles inline and label the plan as lower-assurance.
...
- Preserve multiple adversarial review rounds, using separate Codex sub-agents when available.
